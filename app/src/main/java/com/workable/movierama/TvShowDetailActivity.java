...
public class TvShowDetailActivity extends AppCompatActivity {
    ...
    private TextView similarTvShowsTV;
    private ImageView divider;
    private MaterialFavoriteButton mFavorite;
    private RecyclerView similarTvShows;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        //Setup Similar Tv Show Recycler View
        similarTvShows = findViewById(R.id.similar_movies);
        SimilarTvShowAdapter similarMovieAdapter = new SimilarTvShowAdapter(this);
        LinearLayoutManager linearLayoutManager = new LinearLayoutManager(this,
                RecyclerView.HORIZONTAL, false);
        similarTvShows.setLayoutManager(linearLayoutManager);
        similarTvShows.setItemAnimator(new DefaultItemAnimator());

        similarTvShows.setAdapter(similarMovieAdapter);
        ...
        nameOfTvShow = findViewById(R.id.title);
        ...
        similarTvShowsTV = findViewById(R.id.similar_movies_tv);


        //Fetch Clicked Tv Show Data
        ...
        if (intentThatStartedThisActivity.hasExtra("tv")) {
            ...
            if (tvShow != null) {
                thumbnail = tvShow.getPosterPath();
            }
            if (tvShow != null) {
                movieName = tvShow.getOriginalName();
            }
            ...
            String rating = null;
            if (tvShow != null) {
                rating = Double.toString(tvShow.getVoteAverage());
            }
            String dateOfRelease = null;
            if (tvShow != null) {
                dateOfRelease = tvShow.getFirstAirDate();
            }
            if (tvShow != null) {
                tv_show_id = tvShow.getId();
            }
            List<Integer> genreIds = null;
            if (tvShow != null) {
                genreIds = tvShow.getGenreIds();
            }

            //Restore Favorite Status
            favorites = this.getSharedPreferences("favorites", MODE_PRIVATE);
            if (favorites.contains(String.valueOf(tv_show_id))) {
                mFavorite.setFavorite(favorites.getBoolean(String.valueOf(tv_show_id), false));
            } else {
            ...
            nameOfTvShow.setText(movieName + "\n\n");
            plot.setText("\n" + synopsis);
            userRating.setText("\n" + rating + "/10");
        ...
        mFavorite.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                ...
                if (favorites.getBoolean(String.valueOf(tv_show_id), false)) {
                    //Save Movie ID instead of Title because of title collisions e.g Alladin
                    edit.putBoolean(String.valueOf(tv_show_id), false);
                    mFavorite.setFavorite(false);
                } else {
                    edit.putBoolean(String.valueOf(tv_show_id), true);
        ...
        appBarLayout.addOnOffsetChangedListener(new AppBarLayout.OnOffsetChangedListener() {
            ...
            public void onOffsetChanged(AppBarLayout appBarLayout, int verticalOffset) {
                ...
                if (scrollRange + verticalOffset == 0) {
                    ...
                    nameOfTvShow.setVisibility(View.GONE);
                    genre.setVisibility(View.GONE);
                    releaseDate.setVisibility(View.GONE);
                    isShow = true;
                } else if (isShow) {
                    collapsingToolbarLayout.setTitle(" ");
                    nameOfTvShow.setVisibility(View.VISIBLE);
    ...
    private void loadJSON() {
            ...
            public void onResponse(@NonNull Call<CreditApiResponse> call,
                ...
                if (crew != null) {
                    for (int i = 0; i < crew.size(); i++) {
                        if (crew.get(i).getCrewJob().equals("Executive Producer")) {
        ...
        callSimilarMovies().enqueue(new Callback<TvShowApiResponse>() {
            @Override
            public void onResponse(@NonNull Call<TvShowApiResponse> call,
                ...
                List<TvShow> similarMovies = fetchMovies(response);
                if(similarMovies!=null) {
                    similarMovies.addAll(similarMovies);
                }else{
                    similarTvShows.setVisibility(View.GONE);
                }
            }
    ...
    //Fetch Movies
    private List<TvShow> fetchMovies(Response<TvShowApiResponse> response) {
        TvShowApiResponse similar = response.body();
        if (similar != null) {
            return similar.getTvShows();
        }
        return null;
    }
    ...
    //Get Similar Movies
    private Call<TvShowApiResponse> callSimilarMovies() {
        return movieService.getSimilarTvShows(String.valueOf(tv_show_id),
                BuildConfig.THE_MOVIE_DB_API_TOKEN);
    }

    //Get Reviews
    private Call<ReviewApiResponse> callMovieReviews() {
        return movieService.getTvShowReviews(String.valueOf(tv_show_id),
                BuildConfig.THE_MOVIE_DB_API_TOKEN);
    }

    //Get Credits
    private Call<CreditApiResponse> callCredits() {
        return movieService.getTvShowCredits(String.valueOf(tv_show_id),
                BuildConfig.THE_MOVIE_DB_API_TOKEN);
    }
